...
function addBeadStyle(bead, colorKey) {
    ...
    if (color) {
        if(tissage === TISSAGE.PEYOTE_V) {
            bead.setAttribute("style", "background: linear-gradient(" + color + "bb, " + color + ", " + color + "bb);");
        } else {
            bead.setAttribute("style", "background: linear-gradient(0.25turn, " + color + "bb, " + color + ", " + color + "bb);");
        }
    }
}