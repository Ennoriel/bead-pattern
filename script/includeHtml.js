        ...
        if(!includeElements[i].innerHTML) {
			await fetch('components/' + includeElements[i].getAttribute('src'))
				...
				.then(responseText => {
					...
					[...doc.head.children].filter(e => ['SCRIPT', 'LINK'].includes(e.tagName)).forEach(s => {
						var script = document.createElement(s.nodeName);
						script.innerHTML = s.innerHTML;
						[...s.attributes].forEach(attribute => script.setAttribute(attribute.nodeName, attribute.nodeValue))
